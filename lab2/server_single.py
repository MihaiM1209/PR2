import os, socket, mimetypes, urllib.parse, time
...
def list_files_with_counters():
    ...
    html = f"""<!doctype html><html><head><meta charset='utf-8'><title>Lab 2 — Single Thread</title>
<style>
...
<body>
<header>
  <h1>Lab 2 — Single Thread Server</h1>
  <div class="badge">One request at a time - baseline for performance comparison</div>
</header>
<div class="wrap">
  <div class="card">
    ...
    <p style="margin-top:12px">Variant: <strong>Single Thread</strong></p>
  </div>
</div>
</body></html>"""
    return html
...
def server_loop():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        s.bind((HOST, PORT))
        s.listen(256)
        print(f"[lab2 SINGLE] http://{HOST}:{PORT} serving from {DOCROOT}")
        while True:
            conn, addr = s.accept()
